...
* **高度な Markdown レンダリング**: GitHub Flavored Markdown (GFM)、Highlight.js（コードハイライト）、Mermaid.js（図表・シーケンス図）、KaTeX（数式）をサポートします。
...
```bash
# カレントディレクトリの Markdown を閲覧（デフォルト: http://localhost:8080）
./pjdoc
...
| オプション | デフォルト値 | 説明 |
|---|---|---|
| `-port` | `8080` | Web サーバーのポート番号 |
| `-dir` | `.` | 閲覧対象のルートディレクトリ |
| `-open` | `true` | 起動時にブラウザを自動開閉 |
